...
class ObjectDetctor:
    ...
    def get_segmentation(self, class_id: int) -> set:
        """Get all class_id segmentation results as pixels coordinates in shape (N*M*2) N seg, M pixels"""
        if self.results is None:
            # TODO: or only expose this func and run inference within here?
            # if not, need to add this check to the vis funcs too
            print("Must run inference() before get_segmentations()")
            return np.empty((0))

        # segs = []
        # get all pixel coords that are True
        for i, instance_class in enumerate(self.results["instances"].pred_classes):
            if instance_class == class_id:
                mask = self.results["instances"].pred_masks[i]
                # could yield result here so memory won't explode
                # segs.append(mask.nonzero().tolist())
                yield set([(r, c) for r, c in mask.nonzero().tolist()])
